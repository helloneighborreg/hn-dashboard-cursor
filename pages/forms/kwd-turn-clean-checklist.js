...
export default function KwdTurnCleanChecklistPage({
	...
	async function handleAutoSave({ values, answers, onProgress }) {
		return persistDraft({ values, answers, onProgress });
	}
	...
	async function handleSubmit({ values, answers }) {
		...
		try {
			...
			const json = await persistChecklist({
				...
				onProgress: ({ phase, done, total }) => {
					if (phase === 'upload' && total > 0) {
						setSubmitProgress(`Uploading photos (${done}/${total})… Please keep this page open.`);
					} else if (phase === 'finalize') {
						setSubmitProgress('Finishing submission…');
					}
				},
			});
			if (!json?.data?.id) throw new Error('Could not submit checklist');
...
export const getServerSideProps = requireAuth(async (context) => {
	...
	if (!submission) {
		const propertyCode = defaults[CHECKLIST_IDS.property] || String(query.Property || query.property || '').trim();
		defaults = applyCalculations({
			...defaults,
			[CHECKLIST_IDS.baseCleanFee]: await getBaseCleaningRateForPropertyCode(propertyCode),
		});
	}
	...
	const propertyCode = initialValues[CHECKLIST_IDS.property] || query.Property || query.property || '';
