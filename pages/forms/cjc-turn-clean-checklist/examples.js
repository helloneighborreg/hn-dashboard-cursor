...
function SectionExampleManager({ section, photos, onUploaded, onDeleted }) {
	...
	async function handleFiles(fileList) {
		...
		try {
			for (const file of files) {
				...
				const json = await fetchJson('/api/forms/cjc-turn-clean-checklist/examples', {
					method: 'POST',
					body: {
	...
	async function handleDelete(photoId) {
		...
		setError('');
		try {
			await fetchJson('/api/forms/cjc-turn-clean-checklist/examples', {
				method: 'DELETE',
				body: { id: photoId },
			});
...
export default function CjcChecklistExamplesPage({ initialExamples = {}, dbError = null }) {
	...
	return (
		<Layout title="">
			<Head>
				<title>Manage Example Photos — {CJC_TURN_CLEAN_FORM.name}</title>
			</Head>
			<div className="max-w-3xl mx-auto">
				<div className="mb-6">
					<Link
						href="/forms/cjc-turn-clean-checklist"
...
export const getServerSideProps = requireAuth(async (_context, session) => {
	if (!isAdmin(session.user)) {
		return {
			redirect: {
				destination: '/forms/cjc-turn-clean-checklist',
				permanent: false,
			},
		};
	}
	...
	let dbError = null;
	try {
		initialExamples = await getSectionExamplesByForm(CJC_TURN_CLEAN_FORM_SLUG);
	} catch (err) {
