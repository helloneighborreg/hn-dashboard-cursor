...
export async function applyChecklistSubmissionToTask({
	...
	const patch = buildChecklistTaskPatch(task, { submissionUrl, pdfUrl }, { markCompleted });
	if (!patch) {
	...
	return { ok: true, task: updated, updated: true, notified, pdfUrl };
}
