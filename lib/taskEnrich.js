...
function taskNeedsPropertyLookup(task) {
	const name = task?.property_name?.trim();
	if (name && name !== PLACEHOLDER_PROPERTY) return false;
	return Boolean(task?.property_id) || Boolean(propertyNameFromTitle(task?.title));
}
