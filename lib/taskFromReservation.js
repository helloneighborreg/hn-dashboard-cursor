...
import { getPropertyCode, getReservationCode } from './codes';
...
export function parseReservationCheckout(reservation) {
	const raw = reservation.check_out || reservation.departure_date;
	if (!raw) return { checkoutDate: null, startTime: DEFAULT_CHECKOUT_START };

	const checkoutDate = String(raw).slice(0, 10);
	let startTime = DEFAULT_CHECKOUT_START;
...
export function buildTaskFromReservation(reservation, property) {
	...
	const propertyCode =
		getPropertyCode(property) || reservation.property_name || reservation.property_id || 'Property';
	...
	return {
		...draft,
		title: `${reservationCode} - ${propertyCode}`,
...
export function reservationEligibleForTask(reservation) {
	...
	if (reservation.status === 'cancelled' || reservation.status === 'declined') return false;
