...
export function ownerStatementCashItemKey(property_id, item_id, item_source) {
	return `${property_id}:${item_source || 'bank'}:${item_id}`;
}

function latestLockEntry(map, key, entry, approvedAt) {
...
/** Map approved statement cash items (bank, manual, adjustments) by property/source/id. */
export function mapApprovedStatementCashLocks(approvals = []) {
	const byKey = new Map();
	for (const approval of approvals) {
		const approvedAt = approval.approved_at || '';
		const property_id = approval.property_id;
		if (!property_id) continue;

		for (const tx of approval.statement_data?.transactions || []) {
			if (!tx?.id) continue;
			const source = tx.source || 'bank';
			latestLockEntry(byKey, ownerStatementCashItemKey(property_id, tx.id, source), {
				property_id,
				item_id: tx.id,
				item_source: source,
		...
		for (const adj of approval.statement_data?.adjustments || []) {
			if (!adj?.id) continue;
			latestLockEntry(byKey, ownerStatementCashItemKey(property_id, adj.id, 'adjustment'), {
				property_id,
				item_id: adj.id,
...
/** Map approved statement reservations by property_id:reservation_id. */
export function mapApprovedReservationLocks(approvals = []) {
	const byKey = new Map();
	for (const approval of approvals) {
		const approvedAt = approval.approved_at || '';
		const property_id = approval.property_id;
		...
		const seen = new Set();
		for (const row of approval.statement_data?.reservations || []) {
			if (!row?.id || seen.has(row.id)) continue;
			seen.add(row.id);
			latestLockEntry(byKey, `${property_id}:${row.id}`, {
				property_id,
		...
		for (const reservation_id of approval.reservation_ids || []) {
			if (!reservation_id || seen.has(reservation_id)) continue;
			seen.add(reservation_id);
			latestLockEntry(byKey, `${property_id}:${reservation_id}`, {
				property_id,
				reservation_id,
				approval_id: approval.id,
				statement_period: approval.statement_period || null,
...
export function buildStatementLockMaps({ approvals = [], cashInclusions = [] } = {}) {
	const reservationLocks = mapApprovedReservationLocks(approvals);
	const cashLocks = mapApprovedStatementCashLocks(approvals);

	for (const row of cashInclusions) {
		if (!row?.property_id || !row?.item_id) continue;
		const key = ownerStatementCashItemKey(row.property_id, row.item_id, row.item_source);
		if (!cashLocks.has(key)) {
			cashLocks.set(key, {
	...
	return { reservationLocks, cashLocks };
}

export function statementCashLocksHas(cashLocks, key) {
	if (!cashLocks || !key) return false;
	if (cashLocks instanceof Map) return cashLocks.has(key);
	if (typeof cashLocks === 'object') return Object.prototype.hasOwnProperty.call(cashLocks, key);
	return false;
}

/** Serialize cash lock map for JSON API responses. */
export function serializeStatementCashLocks(cashLocks) {
	if (!cashLocks) return {};
	if (cashLocks instanceof Map) return Object.fromEntries(cashLocks);
	return cashLocks;
}

export function isStatementCashItemLocked(item, cashLocks) {
	if (!item?.id || !item?.property_id || !cashLocks) return false;
	const source = item.kind === 'adjustment' ? 'adjustment' : (item.source || 'bank');
	return statementCashLocksHas(
		cashLocks,
		ownerStatementCashItemKey(item.property_id, item.id, source),
	);
}
...
export function summarizeOwnerStatement(reservations, transactions = [], adjustments = []) {
	...
	// Cash items use bookkeeping signs (expenses negative). On the statement, those
	// charges increase what is due to HN and reduce what is due to the owner.
	const additionalDueToHn = roundMoney(-(transactionTotal + adjustmentTotal));
