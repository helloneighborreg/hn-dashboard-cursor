...
/** Base for management fee % — accommodation, pet, extra guest minus promotions (not resolutions). */
...
export function enrichOwnerStatementReservation(row, { managementFeePercent } = {}) {
	...
	const earningsTotal = calculateManagementFeeBase(row);
	const percent = resolveManagementFeePercent(
		managementFeePercent ?? row.management_fee_percent,
	);
	...
	return {
		...
		management_fee_base: earningsTotal,
...
export function summarizeOwnerStatement(reservations, transactions = [], adjustments = []) {
	...
	return {
		...
		total_due_to_owner: roundMoney(
			reservationTotals.net_owner_income + transactionTotal + adjustmentTotal,
		),
	};
}
