...
import { getReservationCode, resolvePropertyCode } from './codes';
...
function propertyLabel(item) {
	const fromField = resolvePropertyCode(item?.property_name) || item?.property_name?.trim() || '';
...
/** Task list headline — same RESERVATION# - PROPERTY format. */
