import { formatPropertyDisplayName, resolvePropertyCode } from './codes';
import { getFormSubmissionForTask } from './forms/checklistSubmissions';
import { resolveChecklistFormFromSubmission, kwd, cjc } from './forms/checklistFormRegistry';
import { getChecklistFormKey } from './propertyChecklists';
import { getBaseCleaningRateForPropertyCode } from './propertyBaseCleaningRate';
import { taskHeadline } from './taskDisplay';

function checklistFormSlugForTask(task) {
	const key = getChecklistFormKey(resolvePropertyCode(task?.property_name));
	if (key === 'kwd') return kwd.KWD_TURN_CLEAN_FORM_SLUG;
	return cjc.CJC_TURN_CLEAN_FORM_SLUG;
}
...
export function invoiceAmountsFromSubmission(submission) {
	...
	const form = resolveChecklistFormFromSubmission(submission);
	const ids = form.CHECKLIST_IDS;
	const fromCalc = submission?.calculations?.svxB?.value;
	const base = parseAmount(answers[ids.baseCleanFee]?.value);
	const hasAdditional = answers[ids.additionalCharge]?.value === 'Yes';
	const additional = hasAdditional ? parseAmount(answers[ids.additionalAmount]?.value) : 0;
	...
	return {
		...
		additional_description: hasAdditional ? (answers[ids.additionalDetails]?.value || '').trim() || null : null,
	};
}
...
export async function buildBillpayInvoiceFromTask(task, submission = null) {
	const checklist = submission ?? (task?.id
		? await getFormSubmissionForTask(task.id, { formSlug: checklistFormSlugForTask(task) })
		: null);
