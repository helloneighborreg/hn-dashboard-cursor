...
export function sumLeaseAmounts(form) {
	return (
		parseAmountField(form?.rent)
		+ parseAmountField(form?.lease_utilities)
		+ parseAmountField(form?.lease_electric)
		+ parseAmountField(form?.lease_internet)
		+ parseAmountField(form?.lease_parking)
	);
}
...
export function detailsToUtilityForm(details) {
	return {
		...
		checkout_code: textToForm(details?.checkout_code),
