...
/** Current wall-clock date and HH:mm in the business timezone. */
export function nowBusinessClock(timeZone = BUSINESS_TIMEZONE) {
	const now = new Date();
...
export function normalizeClock(value, fallback = '16:00') {
	const s = String(value || fallback).trim().slice(0, 5);
	return /^\d{2}:\d{2}$/.test(s) ? s : fallback;
}

/** True when checkout/due date is still in the future (guest has not checked out yet). */
export function taskCheckoutIsFuture(task, today = todayIso()) {
	const checkout = task?.checkout_date || task?.due_date;
	if (!checkout) return false;
	return checkout > today;
}

/** True when the task due date/time has passed (ignores completion status). */
export function taskIsPastDue(task, now = nowBusinessClock()) {
	if (!task?.due_date) return false;
	const dueTime = normalizeClock(task.due_time);
	if (task.due_date < now.date) return true;
	if (task.due_date > now.date) return false;
	return now.time > dueTime;
}
