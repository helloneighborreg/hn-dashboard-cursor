...
import { taskIsPastDue } from './dates';

export function taskIsOverdue(task) {
	return Boolean(task?.due_date && !isTaskFinished(task) && taskIsPastDue(task));
}

/** UI indicator: overdue → exclamation; else completed / assigned / unassigned. */
