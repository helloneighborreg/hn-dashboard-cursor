...
export const NAV_ITEMS = [
	...
	{ href: FORMS_NAV_PARENT, label: 'Forms', roles: [ROLES.ADMIN], toggleOnly: true },
...
export function navItemsForRole(role, navPermissions) {
	...
	return NAV_ITEMS.filter((item) => {
		if (item.href === '/settings' || item.parent === '/settings') {
			return role === ROLES.ADMIN;
		}
		const itemPerm = permissions[item.href];
		if (itemPerm) return itemPerm[roleKey] === true;
		return item.allUsers || item.roles?.includes(role);
	});
}
