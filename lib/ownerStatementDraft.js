import { applyOwnerStatementItemSelection, isCashItemIncludedInReservations } from './ownerStatementReport';
...
export function buildDraftOwnerStatements(data, selectedReservationIds) {
	...
	const selected = (data.reservations || []).filter((row) => idSet.has(row.id));
	...
	for (const [propertyId, reservations] of byProperty) {
		const template = (data.statements || []).find((s) => s.property_id === propertyId) || {};
