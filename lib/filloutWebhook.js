...
// Note: omit plain `id` — Fillout/Notion adds url param `id` for the dashboard task UUID, not submission id.
const SUBMISSION_ID_KEYS = new Set(['submissionid', 'submission_id']);
const SUBMISSION_URL_KEYS = new Set([
	'editlink',
	'edit_link',
	'submissionurl',
	'submission_url',
	'completed_checklist_url',
...
function assignTarget(target, key, value) {
	...
	else if (normalized === 'id' && !target.taskId) target.taskId = str; // Fillout Notion integration
	...
	else if (SUBMISSION_URL_KEYS.has(normalized) && !target.submissionUrl) target.submissionUrl = str;
}
...
/** Merge Fillout webhook URL query params into the JSON body (Fillout can send IDs either way). */
export function mergeWebhookRequestPayload(body, query = {}) {
...
/** @returns {{ taskId: string|null, reservationId: string|null, submissionId: string|null, submissionUrl: string|null, pdfUrl: string|null }} */
export function parseFilloutWebhookPayload(body) {
	...
	target.taskId = pickFlat(body, 'task_id', 'taskId', 'TaskID') || pickFlat(submission, 'task_id', 'taskId', 'TaskID');
	target.reservationId =
		pickFlat(body, 'reservation_id', 'reservationId', 'ReservationID')
		|| pickFlat(submission, 'reservation_id', 'reservationId', 'ReservationID');
	target.submissionId =
		pickFlat(body, 'submissionId', 'submission_id')
		|| pickFlat(submission, 'submissionId', 'submission_id');
	target.submissionUrl =
		pickFlat(body, 'editLink', 'edit_link', 'submissionUrl', 'submission_url', 'checklist_submission_url')
		|| pickFlat(submission, 'editLink', 'edit_link', 'submissionUrl', 'submission_url', 'checklist_submission_url');
	target.pdfUrl =
	...
	if (submission?.editLink && !target.submissionUrl) {
		target.submissionUrl = String(submission.editLink).trim();
	}
	...
	// Always prefer Fillout's canonical submissionId over any mis-parsed url param.
	if (submission?.submissionId) {
		const canonical = String(submission.submissionId).trim();
		if (!target.submissionId || target.submissionId === target.taskId) {
			target.submissionId = canonical;
		}
	}
...
export function buildCompletionNotes(existingNotes, { submissionId, submissionUrl, pdfUrl }) {
	const lines = [];
	...
	if (submissionUrl) {
		const line = `Completed checklist: ${submissionUrl}`;
		if (!lines.some((l) => l.includes(submissionUrl))) lines.push(line);
	}
	if (pdfUrl) {
