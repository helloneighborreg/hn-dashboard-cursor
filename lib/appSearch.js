...
import { isAdmin, navItemsForRole } from './roles';
...
async function searchTasks(q, user) {
	const supabase = getSupabase();
	const pattern = `%${q}%`;
	let query = supabase
		.from('tasks')
		.select('id, property_name, guest_name, reservation_id, assignee, due_date, status, title')
		.or(`guest_name.ilike.${pattern},property_name.ilike.${pattern},reservation_id.ilike.${pattern},assignee.ilike.${pattern},title.ilike.${pattern}`)
	...
	return (data || []).map((task) => ({
		type: 'task',
		id: task.id,
		label: taskHeadline(task),
		subtitle: [task.assignee, formatDateOrDash(task.due_date)].filter(Boolean).join(' · '),
