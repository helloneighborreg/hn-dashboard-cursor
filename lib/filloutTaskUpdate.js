...
export async function applyFilloutSubmissionToTask(body, options = {}) {
	...
	if (task.status === 'completed' || task.status === 'under_review') {
		...
		if (patch) {
			...
			return {
				...
				task: updated,
				updated: true,
				already_submitted: task.status === 'under_review',
	...
	const updated = await updateTask(task.id, patch, { previousTask: task });
	let submissionNotified = null;
	if (!skipNotify) {
		try {
			submissionNotified = await notifyIfTaskSubmitted(task, updated);
		} catch (err) {
			console.error('Task submission notify failed:', err.message);
		}
	}
	return { ok: true, task: updated, updated: true, submissionNotified };
}
