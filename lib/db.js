...
import { todayIso, taskIsPastDue } from './dates.js';
import { isTaskFinished, sortTasksByDateAsc } from './constants.js';
...
import { taskArchiveStatusPatch } from './taskArchive.js';
...
function throwDbError(error) {
	...
	if (error?.code === '42703') {
		...
		if (/tasks/i.test(msg) && /(assigned_at|started_at|completed_at)/i.test(msg)) {
			throw new Error(
				'Database schema is missing tasks columns required for the task timeline. ' +
					'In Supabase → SQL Editor, run:\n' +
					'- supabase/migrations/20260624_task_timeline.sql',
			);
		}
		if (/tasks/i.test(msg) && /(scheduled_by|paid_at|paid_by)/i.test(msg)) {
			throw new Error(
				'Database schema is missing tasks columns required for payment tracking. ' +
					'In Supabase → SQL Editor, run:\n' +
					'- supabase/migrations/20260704_task_payment.sql',
			);
		}
		if (/tasks/i.test(msg) && /archived_at/i.test(msg)) {
			throw new Error(
				'Database schema is missing tasks columns required for task archiving. ' +
					'In Supabase → SQL Editor, run:\n' +
					'- supabase/migrations/20260627_task_archived.sql',
			);
		}
		throw new Error(
...
export async function getTasks(filters = {}) {
	...
	if (filters.overdue) query = query.lte('due_date', todayIso());
	...
	let rows = data || [];
	if (filters.overdue) {
		rows = rows.filter((task) => !isTaskFinished(task) && taskIsPastDue(task));
	}
	if (filters.exclude_overdue) {
		rows = rows.filter((task) => isTaskFinished(task) || !taskIsPastDue(task));
	}
...
export async function updateTask(id, updates, { previousTask } = {}) {
	...
	if (previousTask) {
		Object.assign(patch, taskArchiveStatusPatch({ ...previousTask, ...patch }));
	}
...
/** Assigned, incomplete tasks past due that have not yet triggered an overdue notification. */
export async function getTasksPendingOverdueNotify() {
	const supabase = getSupabase();
	const { data, error } = await supabase
		.from('tasks')
		.select('*')
	...
	if (error) throwDbError(error);
	return (data || []).filter((task) => taskIsPastDue(task));
}

export async function markTaskOverdueNotified(id) {
	const supabase = getSupabase();
	const { error } = await supabase
		.from('tasks')
		.update({ overdue_notified_at: new Date().toISOString(), updated_at: new Date().toISOString() })
		.eq('id', id);
	if (error) throwDbError(error);
}
...
export async function insertPropertyChangeLog(entry) {
	const supabase = getSupabase();
	const { data, error } = await supabase
		...
		.insert({
			...
			edited_by_username: entry.edited_by_username,
...
export async function getOwnerStatementCashInclusions(propertyIds) {
	const ids = uniquePropertyIds(propertyIds);
	if (!ids.length) return [];
	const supabase = getSupabase();
	const { data, error } = await supabase
...
export async function getOwnerStatementCashInclusionForItem(property_id, item_id, item_source) {
	const supabase = getSupabase();
	const { data, error } = await supabase
		.from('owner_statement_cash_inclusions')
		.select('*')
...
function resolveStatementLockMonth(statement, date_from) {
	if (date_from) return String(date_from).slice(0, 7);
	const first = (statement.reservations || []).find((row) => row.check_in);
	if (first?.check_in) return String(first.check_in).slice(0, 7);
	const firstTx = (statement.transactions || []).find((row) => row.date);
	if (firstTx?.date) return String(firstTx.date).slice(0, 7);
	return null;
}

async function lockApprovedStatementItems(statement, date_from) {
	const property_id = statement.property_id;
	if (!property_id) return;

	const statement_month = resolveStatementLockMonth(statement, date_from);
	if (!statement_month) return;
	...
	for (const tx of statement.transactions || []) {
		if (!tx?.id) continue;
		await setOwnerStatementCashInclusion({
			property_id,
			item_id: tx.id,
			item_source: tx.source || 'bank',
			statement_month,
			included: true,
		});
	}
...
export async function saveOwnerStatementApprovals(statements, {
	...
	for (const statement of statements) {
		...
		await lockApprovedStatementItems(statement, date_from);
		results.push(data);
	}
...
async function deleteOwnerStatementPdf(storagePath) {
...
async function unlockApprovedStatementItems(row) {
	const statement = row.statement_data || {};
	const property_id = row.property_id;
	if (!property_id) return;

	const reservationIds = new Set();
	for (const reservation of statement.reservations || []) {
		if (reservation?.id) reservationIds.add(reservation.id);
	}
	for (const reservation_id of row.reservation_ids || []) {
		if (reservation_id) reservationIds.add(reservation_id);
	}
...
export async function voidOwnerStatement(id) {
	...
	await unlockApprovedStatementItems(row);
	await deleteOwnerStatementPdf(row.pdf_storage_path);
	...
	return { id, property_id: row.property_id };
}
