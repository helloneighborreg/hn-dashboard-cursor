...
import { sortTasksByDateAsc, countTasksByIndicator } from './constants.js';
...
export async function getTasks(filters = {}, { columns = '*' } = {}) {
	const supabase = getSupabase();
...
export async function getTasksForToday(options = {}) {
	return getTasks({
		due_date: todayIso(),
...
/** Nav/dashboard counts — one lightweight query beats several head-only round trips on Supabase. */
export async function getTaskIndicatorCounts({ assignee } = {}) {
	const supabase = getSupabase();
	let query = supabase.from('tasks').select('status, assignee, due_date');
	if (assignee) query = query.eq('assignee', assignee);

	const { data, error } = await query;
	if (error) throwDbError(error);
	return countTasksByIndicator(data || []);
}
