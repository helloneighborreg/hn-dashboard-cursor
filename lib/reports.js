...
import {
	...
	getOwnerStatementCashInclusions,
...
export async function buildOwnerStatementsReport(filters) {
	...
	const [noteRows, statementApprovals, cashInclusionRows] = await Promise.all([
		getOwnerStatementNotes(ownerPropertyIds),
		getOwnerStatementApprovalsForProperties(ownerPropertyIds),
		getOwnerStatementCashInclusions(ownerPropertyIds),
	]);
	const { reservationLocks, cashLocks } = buildStatementLockMaps({
		approvals: statementApprovals,
		cashInclusions: cashInclusionRows,
	});
	...
	const reservationsWithInclusion = enrichedReservations.map((row) => {
		...
		const assignedMonth = inclusion?.property_id === row.property_id
			...
			: null;
		const lock = reservationLocks.get(`${row.property_id}:${row.id}`) || null;
		return {
			...row,
	...
	const additionalItems = selectedPropertyIds.flatMap((propertyId) => {
		...
		return entry.transactions
			.filter((row) => !isStatementCashItemLocked(
	...
	const statementTemplates = selectedPropertyIds.map((propertyId) => {
		const property = propertyById[propertyId];
		if (!property) return null;
		return buildPropertyOwnerStatement({
			property,
	...
	return {
		...
		statements,
		statement_templates: statementTemplates,
		additional_items: additionalItems,
		statement_cash_locks: serializeStatementCashLocks(cashLocks),
	};
}
