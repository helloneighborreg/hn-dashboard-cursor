...
import { filterVisibleProperties, isHiddenPropertyId } from './hiddenProperties.js';
import { reservationActsAsCancelled } from './reservationDates.js';

export { getPropertyCode, getPropertyDisplayName, getPropertyFullName, getReservationCode } from './codes';
...
async function request(path, params = {}) {
	...
	if (!token) {
		throw new Error(
			'HOSPITABLE_API_TOKEN is not set. Add it in Cloudflare → Workers → hn-dashboard-cursor → Settings → Variables and Secrets, then redeploy.',
		);
	}
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) {
      throw new Error(
        'Hospitable API: Unauthenticated. In Cloudflare, set HOSPITABLE_API_TOKEN to a valid Personal Access Token from Hospitable → Settings → API access, then redeploy.',
      );
    }
...
export async function getProperties({ include = 'details' } = {}) {
  ...
  return filterVisibleProperties(data.data || []);
}

export async function getProperty(id) {
  if (isHiddenPropertyId(id)) {
...
export async function getPropertyImages(id) {
  if (isHiddenPropertyId(id)) return [];
  try {
...
/** First listing/gallery image, or property picture fallback. */
export async function getPropertyPrimaryImageUrl(id) {
  if (!id || isHiddenPropertyId(id)) return null;
  try {
    const images = await getPropertyImages(id);
    const first = images[0];
    const fromGallery = first?.url || first?.original;
