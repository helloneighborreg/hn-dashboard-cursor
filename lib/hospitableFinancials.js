...
export function normalizeFeeColumnLabel(rawLabel) {
	...
	if (/^accommodations?\s+tax\b/i.test(label)) return 'Lodging Taxes';
...
function mergeGuestTaxesIntoHostFees(fees, guestFin) {
	if ((fees['Lodging Taxes'] || 0) > 0) return fees;
	let total = 0;
	for (const tax of guestFin?.taxes || []) {
		const column = normalizeFeeColumnLabel(feeLabel(tax));
		if (column === 'Lodging Taxes') {
			total += Math.abs(cents(tax));
		}
	}
	if (total > 0) fees['Lodging Taxes'] = total;
	return fees;
}

/** Host financials for owner statements — includes guest-side taxes when host taxes are absent. */
export function parseOwnerStatementFinancials(financials) {
	const host = parseHostFinancials(financials?.host);
	mergeGuestTaxesIntoHostFees(host.fees_by_label, financials?.guest);
	return host;
}
