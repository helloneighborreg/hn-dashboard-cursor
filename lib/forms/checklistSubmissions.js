...
function buildCalculations(answers) {
	return {
		svxB: {
			name: 'Invoice Total',
			value: Number(answers?.['3sV8']?.value) || 0,
		},
	};
}
...
function submissionRow({
	...
	return {
		form_id: CJC_TURN_CLEAN_FORM_ID,
		form_slug: CJC_TURN_CLEAN_FORM_SLUG,
		task_id: taskId || null,
		...
		answers,
		calculations: buildCalculations(answers),
...
export async function saveFormSubmissionDraft({
	...
	if (submissionId) {
		...
		return updateFormSubmission(submissionId, {
			...
			calculations: buildCalculations(mergedAnswers),
	...
	if (reservationId) {
		const existing = await getFormSubmissionForReservation(reservationId);
		if (existing) {
			assertSubmissionEditable(existing);
			const mergedAnswers = mergeSubmissionAnswers(existing.answers, answers);
	...
	if (taskId) {
		const existing = await getFormSubmissionForTask(taskId);
		if (existing) {
			assertSubmissionEditable(existing);
			const mergedAnswers = mergeSubmissionAnswers(existing.answers, answers);
			return updateFormSubmission(existing.id, {
...
export function buildSubmissionViewUrl(submissionId) {
	return `/forms/cjc-turn-clean-checklist?submission_id=${submissionId}`;
}
