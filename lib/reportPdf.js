...
import { OWNER_STATEMENT_MANAGER, OWNER_STATEMENT_MANAGER_ADDRESS, OWNER_STATEMENT_HN_TOTAL_LABEL, formatAddressTwoLines, ownerStatementPdfFilename, statementAdjustmentsTotal } from './ownerStatementReport';

const PDF_MARGIN = 14;
const PDF_LANDSCAPE_WIDTH = 297;
const PDF_LANDSCAPE_HEIGHT = 210;
const STATEMENT_NOTES = [
	'Commission: Commission is charged on the accommodation after discounts are removed. Channel fees do not impact your commission.',
	'Nights: Represents the total nights that were recognized on this statement according to your revenue recognition.',
];
...
function renderOwnerStatementPage(doc, autoTable, statement, { managerName, logoHorizontalDataUrl }) {
	...
	doc.text('OWNER STATEMENT', PDF_MARGIN, y);
	y += 5;

	doc.setTextColor(30);
	doc.setFont('helvetica', 'bold');
	doc.setFontSize(16);
	doc.text(statement.statement_period || 'Statement', PDF_MARGIN, y);
	y += 8;
	...
	const businessTop = PDF_MARGIN + logoHeight + 3;
	doc.setFont('helvetica', 'normal');
	doc.setFontSize(9);
	doc.setTextColor(30);
	...
	doc.setFontSize(8);
	doc.setTextColor(80);
	doc.text(OWNER_STATEMENT_MANAGER_ADDRESS.line1, rightEdge, businessTop + 5, { align: 'right' });
	doc.text(OWNER_STATEMENT_MANAGER_ADDRESS.line2, rightEdge, businessTop + 9, { align: 'right' });

	y = Math.max(propertyBlockBottom, businessTop + 14) + 4;
	...
	if (reservations.length) {
		...
		reservationBody.push([
			...
			{ content: fmtStatement$(totals.total_net_revenue), styles: { fontStyle: 'bold', halign: 'right' } },
	...
	autoTable(doc, {
		...
		body: reservationBody.length ? reservationBody : [[
			{ content: 'No reservations for this period.', colSpan: 9, styles: { halign: 'center', textColor: [120, 120, 120] } },
		]],
		styles: { fontSize: 7, cellPadding: 1.5, overflow: 'linebreak' },
		headStyles: { fillColor: [245, 245, 245], textColor: [30, 30, 30], fontStyle: 'bold', halign: 'center' },
		columnStyles: {
			0: { cellWidth: 16 },
			1: { cellWidth: 20 },
			2: { cellWidth: 14 },
	...
	doc.setFontSize(10);
	doc.setTextColor(30);
	doc.text('TRANSACTIONS', PDF_MARGIN, y);
	y += 4;
	...
	if (!transactions.length) {
		...
		doc.setFontSize(8);
		doc.setTextColor(120);
		doc.text('There are no transactions to report for this month.', PDF_MARGIN, y);
		y += 8;
	} else {
		autoTable(doc, {
			startY: y,
	...
	doc.setFontSize(10);
	doc.setTextColor(30);
	doc.text('ADJUSTMENTS', PDF_MARGIN, y);
	y += 4;
	...
	if (!adjustments.length) {
		...
		doc.setFontSize(8);
		doc.setTextColor(120);
		doc.text('There are no adjustments for this period.', PDF_MARGIN, y);
		y += 8;
	} else {
		autoTable(doc, {
			startY: y,
	...
	const summaryItems = [
		['Net Booking Revenue', statement.totals?.total_net_revenue],
		['Management', statement.totals?.reservation_commissions_to_manager],
		['Cleaning', statement.totals?.total_cleaning_fee],
		[OWNER_STATEMENT_HN_TOTAL_LABEL, statement.totals?.total_owed_to_manager],
	...
	const summaryX = pageWidth - PDF_MARGIN - 88;
	doc.setFont('helvetica', 'normal');
	doc.setFontSize(9);
	doc.setTextColor(30);
	let summaryY = y + 4;
	for (const [label, amount] of summaryItems) {
		...
		doc.text(fmtStatement$(amount), pageWidth - PDF_MARGIN, summaryY, { align: 'right' });
		summaryY += 5;
	}

	const barY = summaryY + 2;
	doc.setFillColor(30, 30, 30);
	doc.rect(PDF_MARGIN, barY, contentWidth, 9, 'F');
	doc.setFont('helvetica', 'bold');
	doc.setFontSize(10);
	doc.setTextColor(255, 255, 255);
	doc.text(dueToOwnerLabel, PDF_MARGIN + 3, barY + 6);
	doc.text(fmtStatement$(statement.totals?.total_due_to_owner), pageWidth - PDF_MARGIN - 3, barY + 6, { align: 'right' });
	...
	if (statement.notes?.trim()) {
		...
		doc.setFontSize(8);
		doc.setTextColor(80);
		const noteLines = doc.splitTextToSize(statement.notes.trim(), contentWidth);
