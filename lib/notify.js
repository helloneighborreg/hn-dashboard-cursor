...
export function taskBecameUnderReview(before, after) {
	return before?.status !== 'under_review' && after?.status === 'under_review';
}

function submissionMessage(task) {
	const lines = [
		'Hello Neighbor — checklist submitted for review',
		'',
		task.title,
		`Property: ${task.property_name || '—'}`,
		`Guest: ${guestLabel(task)}`,
		...
		`Due: ${formatDue(task)}`,
		'',
		'Approve this task in the dashboard when the checklist looks good.',
	];
	return lines.join('\n');
}

/** Email admins when a cleaner submits a checklist for review. */
export async function notifyTaskSubmitted(task) {
	const recipients = getAdminNotifyEmails();
	if (!recipients.length) {
		return { emailed: false, skipped: true, reason: 'no_recipients' };
	}
	if (!emailConfigured()) {
		console.warn('Task submission notify: set RESEND_API_KEY and TASK_NOTIFY_FROM_EMAIL to send email');
		return { emailed: false, skipped: true, reason: 'email_not_configured' };
	}

	const subject = `Checklist submitted: ${task.title}`;
	const emailed = await sendEmail(recipients, subject, { text: submissionMessage(task) });
	return {
		emailed,
		skipped: !emailed,
		reason: emailed ? null : 'delivery_failed',
...
/** Enrich task row and email admins when status transitions to under_review. */
export async function notifyIfTaskSubmitted(previousTask, updatedTask) {
	if (!taskBecameUnderReview(previousTask, updatedTask)) return null;

	try {
		const [enrichedRow] = await enrichTasks([updatedTask]);
		const enriched = withChecklistUrl(enrichedRow);
		return await notifyTaskSubmitted(enriched);
	} catch (err) {
		console.error('Task submission notify failed:', err.message);
