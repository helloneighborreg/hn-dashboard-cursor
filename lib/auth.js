...
const REMEMBERED_USERNAME_COOKIE = 'hn_remembered_username';
const REMEMBERED_USERNAME_MAX_AGE = 60 * 60 * 24 * 365; // 1 year
...
/** Read remembered username from request cookies (works in SSR and API routes). */
export function getRememberedUsername(req) {
	const raw = req?.headers?.cookie || '';
	const match = raw.match(new RegExp(`(?:^|;\\s*)${REMEMBERED_USERNAME_COOKIE}=([^;]*)`));
	if (!match) return '';
	try {
...
/** Persist username for login convenience (not a secret — password is never stored). */
export function setRememberedUsernameCookie(res, username) {
	const value = String(username || '').trim();
	if (!value) return;
	const secure = isProduction() ? '; Secure' : '';
	appendSetCookie(
		res,
		`${REMEMBERED_USERNAME_COOKIE}=${encodeURIComponent(value)}; Path=/; Max-Age=${REMEMBERED_USERNAME_MAX_AGE}; SameSite=Lax${secure}`,
	);
}

export function clearRememberedUsernameCookie(res) {
	const secure = isProduction() ? '; Secure' : '';
	appendSetCookie(
		res,
		`${REMEMBERED_USERNAME_COOKIE}=; Path=/; Max-Age=0; SameSite=Lax${secure}`,
	);
}
