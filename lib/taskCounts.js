...
import { TASK_TAB_ORDER, TASK_TAB_PATHS } from './taskRoutes';

export const EMPTY_TASK_COUNTS = Object.fromEntries(
	TASK_TAB_ORDER.map((key) => [key, 0]),
);
...
/** Seed sidebar counts from dashboard stats (avoids a duplicate /api/tasks round-trip). */
export function taskCountsFromDashboardStats(stats) {
	if (!stats) return null;
	return {
		unassigned: stats.tasks_unassigned ?? 0,
		assigned: stats.tasks_assigned ?? 0,
		completed: stats.tasks_completed ?? 0,
		under_review: stats.tasks_under_review ?? 0,
