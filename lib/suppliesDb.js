...
import { createExpense, isDbConnectivityError, DB_UNREACHABLE_MESSAGE } from './db.js';
...
/** @deprecated Use getActiveSupplyOrders */
export async function getActiveSupplyOrder() {
	const orders = await getActiveSupplyOrders();
	return orders.find((o) => o.status === SUPPLY_ORDER_STATUS.DRAFT) || orders[0] || null;
}
...
export async function markSupplyOrderPaid(orderId, paidBy) {
	...
	const expense = await createExpense({
		...
		notes: `Supply order invoice${order.notes ? ` — ${order.notes}` : ''}`,
		receipt_url: null,
	});
