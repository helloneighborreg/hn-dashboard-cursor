...
export async function backfillFilloutTasks({ dryRun = false, verbose = true } = {}) {
	...
	for (const [formKey, formId] of Object.entries(formIds)) {
		...
		for await (const submission of iterateFilloutSubmissions(formId, { includeEditLink: true })) {
			...
			try {
				...
				if (result.updated) {
					...
					if (result.task?.checklist_submission_url) stats.submission_url_added += 1;
					if (result.task?.status === 'completed') stats.completed += 1;
					log(`  ✓ ${label} → ${result.task.status}${result.task.checklist_submission_url ? ' + checklist' : ''}${result.task.checklist_pdf_url ? ' + PDF' : ''}`);
				}
			} catch (err) {
